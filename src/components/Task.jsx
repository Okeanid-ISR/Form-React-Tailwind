    export const Task = ({indx, children, uniqueId, isChecked, completed, onChange, onRemove}) => {
        const handleInputChange = (event) => {
            onChange(event, children, indx, uniqueId);
            localStorage.setItem(uniqueId, event.target.checked);
        }
            ...
            <div className="container mx-auto justify-start w-full max-w-xl bg-white px-6 py-2">
                <div className="container border rounded-md p-4 mx-auto flex flex-row justify-between items-center justify w-full max-w-xl">
                    <div className="flex space-x-2">
                        ...
                        <h3 style={{color: completed ? 'black' : '#A1A1AA'}} className="text-gray-400 textName">
                            {indx}. {children}
                        </h3>
                    </div>
